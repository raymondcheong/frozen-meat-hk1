...
const { mergeNewsArticles, buildNewsOutput } = require('./news-utils.cjs');
const { stripHtml, localizeNewsArticle } = require('./overseas-zh-tw.cjs');
const { isUsableSourceText } = require('./article-summary.cjs');
...
async function fetchArticleExcerpt(url, maxChars = 2000) {
  if (!url) return '';
  try {
    ...
    $('script, style, nav, footer, aside, header, .comments, .related, .share').remove();
    const selectors = [
    ...
    for (const sel of selectors) {
      $(sel).each((_, el) => {
        ...
        if (t.length > 35 && !/^https?:\/\//i.test(t)) paras.push(t);
      });
      if (paras.length >= 4) break;
    }
  return paras.slice(0, 6).join(' ').substring(0, maxChars);
  } catch (err) {
    console.warn(`  [正文] 略過 ${url}: ${err.message}`);
    return '';
  }
}

function pickRssBody($, el) {
  const encoded =
    $(el).find('content\\:encoded').text().trim() ||
    $(el).find('encoded').text().trim() ||
    '';
  const description = $(el).find('description').text().trim();
  const raw = encoded || description;
...
async function fetchRSS(source) {
  ...
  try {
    ...
    for (const item of items) {
      if (!isUsableSourceText(item.keyInfo.briefReason) && item.sourceUrl) {
        const excerpt = await fetchArticleExcerpt(item.sourceUrl);
        if (isUsableSourceText(excerpt)) {
          item.keyInfo.briefReason = excerpt;
          const merged = `${item.title} ${excerpt}`;
          item.keyInfo.products = detectProducts(merged);
          item.keyInfo.regions = detectRegions(merged);
        }
      }
      articles.push(item);
    }
...
// ===== 主函數 =====
async function scrapeRssArticles() {
  let allArticles = [];
...
async function main() {
  ...
  const allArticles = await scrapeRssArticles();
  
  console.log(`\n總共抓取 ${allArticles.length} 條 RSS 新聞`);

  const fallbackArticles = allArticles.length === 0 ? getDefaultArticles() : [];
  if (fallbackArticles.length) {
    console.log('RSS 未獲取到新聞，將保留既有文章或使用默認列表（不含 FEHD，由 market-data 生成）');
  }

  const mergedArticles = mergeNewsArticles(allArticles, fallbackArticles).map((article) => {
    if (article.id?.startsWith('fehd-')) return article;
    return localizeNewsArticle(article);
  });

  // 生成統計數據
  const outputData = buildNewsOutput(mergedArticles);
  ...
  console.log(`\n✅ 新聞數據已保存: ${outputPath}`);
  const { summary } = outputData;
  const fehdPinned = mergedArticles[0]?.id?.startsWith('fehd-') ? '（含 FEHD 置頂）' : '';
  console.log(
    `📊 總文章: ${summary.totalArticles} | 重大影響: ${summary.highImpactCount} | 價格變動: ${summary.priceChangeCount} | 關稅: ${summary.tariffRelatedCount} | 物流: ${summary.logisticsRelatedCount} ${fehdPinned}`
  );
}
...
module.exports = { scrapeRssArticles, getDefaultArticles };
